...
class WS {
    ...
    // 处理Websocket超时(在 websocket 连接被建立后，如果一段时间未活动，服务器或防火墙可能会超时或终止连接)
    // 注意：需要在onOpen后调用keepAlive
    keepAlive() {
        const timeout = 20000;
        if (this.ws.readyState == this.ws.OPEN) {
            this.ws.send('');
        }
        timeId = setTimeout(this.keepAlive, timeout);
    }
    // 注意：需要在onClose后调用cancelKeepAlive
    cancelKeepAlive() {
        if (this.timeId) {
